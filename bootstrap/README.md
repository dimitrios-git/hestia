...
bootstrap/
  ...
  roles/
    ...
    nvidia/             # opt-in proprietary NVIDIA driver from non-free (become; default off; nvidia_backports flip ready for a future >= 555 explicit-sync driver)
...
| `enable_minipro` | `minipro` | `minipro: [pkg-config, libusb-1.0-0-dev, zlib1g-dev]` | build + install **minipro** (TL866xx/T48/T56 EEPROM programmer CLI) from source (**default off**, hardware-specific). It is packaged in **neither Debian nor Ubuntu** — verified 2026-08-09 — so `apt install minipro` fails everywhere and source is the only path; see `roles/minipro/README.md` |
| `enable_android_dev` | `android_studio` | `android: [openjdk-21-jdk, cpu-checker]` | Android development — JDK 21 + `cpu-checker` (apt; 17 isn't in Debian trixie's apt at all, only 21/25), **Android Studio** itself from a pinned/sha256-verified vendor tarball into `~/.local/share/android-studio` (no apt package exists), and the `kvm` group (needs a re-login) for accelerated emulation (**default off** — per-host dev-toolchain choice). See `roles/android_studio/README.md` |
| `enable_tci` | `blastem` + `gendev` (both opt-in via this same toggle, no flag of their own) | `tci: [libsdl2-image-dev, libsdl2-ttf-dev, gdb, valgrind, alsa-utils, nethack-console, python3-pil, retroarch, retroarch-assets, libretro-snes9x, mupen64plus-qt, libfuse2t64]` + `localbin_binaries: flycast` | thecodingidiot chapter prerequisites (**default off**). Derived from the chapters' own Setup pages (canonical: stoa `apps/thecodingidiot/content/en-GB/**`); re-derive with a `grep -rE 'apt(-get)? install'` over that tree — last re-derived 2026-09-04 against three newly-written chapters (`c08-the-deadlock`, `r01-the-scaler`, `f02-version-control`): no new entries needed, every apt line in their Setup pages was already covered by an earlier chapter's entry. `minipro` (f03b) is **not** here — it isn't in Debian at all and needs a source build. `python3-pil` is Pillow's actual Debian package name (`python3-pillow` does not exist) — needed by g01b/g02a's `gen_assets.sh` scripts (not a literal `apt install` line, so it doesn't show up via the grep above) and by r01's billboard-sprite generator (which **is** a literal grep-derived line there). Also NOT grep-derived: a **console-emulator set** for testing tci's own homebrew across the four systems it targets, all reachable with **NO console or BIOS dump** — **`blastem` role** (Genesis/Mega Drive — deliberately **not** an apt entry: r02-the-scaler's own Setup page, verified directly, says Debian's `blastem` package only resolves its config correctly through dpkg's own install process and to use the official tarball instead; see `roles/blastem/README.md` for the course-correction from the apt package hestia originally shipped in #331/#332), **retroarch** + **libretro-snes9x** (SNES — no standalone Snes9x/bsnes apt package exists, only libretro cores), **mupen64plus-qt** (N64 — replaces PS1 in the curriculum: every apt-packagable PS1 emulator, mednafen and DuckStation included, hard-requires a real dumped retail BIOS with no HLE fallback; N64 has used HLE PIF/boot for 20+ years and needs no BIOS file at all), and **flycast** (Dreamcast — no apt package exists at all, so it's a pinned `localbin_binaries` AppImage instead, needing `libfuse2t64` to run; ships a built-in HLE BIOS enabled by default, a real dumped one is optional). Also rides this toggle: **`gendev` role** — m68k-elf GCC 9.3.0 + SGDK (pinned vendor `.deb` into `/opt/gendev`, not in Debian apt at all) for r02-the-scaler's Mega Drive build; see `roles/gendev/README.md` |
