...
- **`custom/weather`** (`user/waybar/scripts/weather.sh`, symlinked) shows the condition emoji + temperature in the bar (the emoji is wrapped in the standard `<span size='xx-large' rise='-3072'>` icon span so the temp aligns with the other modules' text; needs `fonts-noto-color-emoji`, installed) and location/condition/feels-like/humidity/wind in the tooltip, from **wttr.in** (`curl`, **IP-geolocated** — no location configured). Refreshes every **30 min** (`interval` — wttr.in etiquette) and **caches** the last good reading to `$XDG_RUNTIME_DIR/waybar-weather.json`, falling back to it on a network failure (prints nothing → module hidden if there's no data at all). `weather.sh show` (the on-click) opens the full forecast.
...
- **Built-in tooltips:** `memory` hover shows used/total/free + swap (`tooltip-format` with `{avail}`/`{swap*}`); `clock` hover shows the full date (`%A, %d %B %Y`, bold) + a **native month calendar** (`{calendar}` in `<tt>`, today accented `#ce0056`) — waybar v0.12, no external `cal`. (No `actions`/`on-click`, deliberately — they'd show a misleading hand cursor with nothing to click; re-add `actions` `on-scroll` if you want month browsing back.)
