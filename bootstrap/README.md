...
| `enable_tci` | *(none — package-only)* | `tci: [libsdl2-image-dev, libsdl2-ttf-dev, gdb, valgrind, alsa-utils, nethack-console, python3-pil, blastem, retroarch, retroarch-assets, libretro-snes9x, mednafen, mednaffe, libfuse2t64]` + `localbin_binaries: flycast` | thecodingidiot chapter prerequisites (**default off**). Derived from the chapters' own Setup pages (canonical: stoa `apps/thecodingidiot/content/en-GB/**`); re-derive with a `grep -rE 'apt(-get)? install'` over that tree. `minipro` (f03b) is **not** here — it isn't in Debian at all and needs a source build. `python3-pil` is Pillow's actual Debian package name (`python3-pillow` does not exist) — needed by g01b/g02a's `gen_assets.sh` scripts, which aren't a chapter `apt install` line so this one doesn't show up via the grep above. Also NOT grep-derived: a **console-emulator set** for testing tci's own homebrew across the four systems it targets — **blastem** (Mega Drive, standalone with its own debugger, no BIOS needed), **retroarch** + **libretro-snes9x** (SNES — no standalone Snes9x/bsnes apt package exists, only libretro cores), **mednafen** + **mednaffe** (PS1 — needs a legally-dumped BIOS in `~/.mednafen/firmware/`, hestia can't ship one), and **flycast** (Dreamcast — no apt package exists at all, so it's a pinned `localbin_binaries` AppImage instead, needing `libfuse2t64` to run and a legally-dumped BIOS in `~/.local/share/flycast/data/`) |
