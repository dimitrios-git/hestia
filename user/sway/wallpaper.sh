...
# 2026-07): per-output plain-mesh loop videos via mpvpaper. Run from the
...
#
# Assets: ~/.local/share/backgrounds/hestia/plain-mesh-<variant>-<WxH>.mp4
# (the `wallpapers` role downloads them; static .png companions ship too, for
# wpaperd/swaybg use). Per output: exact resolution match, else the largest
# available (mpv scales cleanly).
...
pick_file() {  # $1 W, $2 H -> best plain-mesh mp4 for this variant
    local exact="$DIR/plain-mesh-$variant-${1}x${2}.mp4"
    if [ -f "$exact" ]; then echo "$exact"; return; fi
    local best="" best_px=0 f wh w h
    for f in "$DIR"/plain-mesh-"$variant"-*.mp4; do
        [ -f "$f" ] || continue
