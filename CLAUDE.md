...
- **Repo → distributable spin roadmap.** §5 path generalisation **done** (every host-specific path resolves from a var). §6 role **feature-flags done** (`enable_*` toggles gate the optional roles). Remaining: §6 **inputs front-end** (collect a user's answers → write host_vars/toggles; leaning an answers file), optional package-group gating, and §7 the layered `users/`/`defaults/` migration — tracked in **`docs/repo-structure-design.md` §6–§7** (the single source; don't re-list it here). Phased on purpose — don't big-bang the structural move.
