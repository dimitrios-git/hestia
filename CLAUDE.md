...
- **Repo → distributable spin roadmap.** §5 path generalisation **done**. §6 **done**: role **feature-flags** (`enable_*` toggles) + the **inputs front-end** (`bootstrap/setup.sh` writes `host_vars/localhost.yml` from the user's answers). Remaining: optional **package-group gating** (skip the `samba` apt group when `enable_samba=false`), **identity-as-inputs** (git name/email/key + `credential-unlock.sh` keygrip are still hardcoded literals), and §7 the layered `users/`/`defaults/` migration — tracked in **`docs/repo-structure-design.md` §6–§7** (the single source; don't re-list it here). Phased on purpose — don't big-bang the structural move.
