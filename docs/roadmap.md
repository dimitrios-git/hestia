...
| Category | Default | Gates | Note |
|---|---|:---:|---|
| Terminal emulator | Alacritty (+ kitty, themed runner-up) | `●●●◐●●` | VERDICT (2026-08): Alacritty retakes the default from kitty — kitty was only ever picked because Alacritty had no image-rendering protocol ranger's previews needed, and vifm (the file-manager winner) doesn't need one at all. Alacritty's built-in vi mode (remapped onto vim's own `Visual`/`Search`/`CurSearch` highlight groups, not just accent-coloured) was the deciding feature, live-verified by scripting real keystrokes into it. kitty is kept + themed (no cited flaw — still needed for ranger's image previews and the `$mod+m` music session, rebuilt on tmux). **Researched gate is partial (◐), honestly:** only the kitty-vs-Alacritty half of the four-contender field actually got trialled — foot and GNOME Terminal remain untested, kept in the opt-in `enable_terminal_emulators` tail rather than given a fabricated verdict. Full trial in [showcase/terminal-emulator.md](../showcase/terminal-emulator.md) |
