...
# this NVIDIA/wlroots setup (:hover state changes — links, transparent divs);
# it's the same shape of bug as Steam's native-Wayland client backend (see the
# `steam` role), a Chromium-family app's own Wayland integration being unstable
# on wlroots. --disable-gpu-compositing fixes a SEPARATE flicker that only
# showed up once forced onto XWayland (live-confirmed 2026-08-10) — this is
...
exec env PATH="/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin" chromium --ozone-platform=x11 --disable-gpu-compositing "$@"
