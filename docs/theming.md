...
| bat | `.tmTheme` syntax theme | ✅ | `user/bat/themes/wildcharm.tmTheme`, rendered from `palette.yml`'s **`syntax:`** table (realigned M4; mirrors the shared TextMate scope map used by the web Shiki pair — change scope decisions in both, colours in `palette.yml` first); compiled into bat's cache by the dotfiles role (`bat cache --build`); drives vifm's code preview. Debian ships the binary as `batcat` |
