...
- **Click handlers** open floating TUIs as toggles (`pgrep … && pkill … || kitty --class floatterm -e …`) so a second click closes the popup:
  ...
  - `custom/weather` → toggles the full multi-day forecast (`weather.sh show` → `curl wttr.in | less -R`) in a floatterm.
...
- **`custom/weather`** (`user/waybar/scripts/weather.sh`, symlinked) shows the condition emoji + temperature in the bar (emoji at natural size — needs `fonts-noto-color-emoji`, installed; the temperature is wrapped in a `<span rise='…'>` to drop it onto the other modules' text baseline, since the emoji's tall glyph otherwise rides it too high — a hand-tuned offset) and location/condition/feels-like/humidity/wind in the tooltip, from **wttr.in** (`curl`, **IP-geolocated** — no location configured). Refreshes every **30 min** (`interval` — wttr.in etiquette) and **caches** the last good reading to `$XDG_RUNTIME_DIR/waybar-weather.json`, falling back to it on a network failure (prints nothing → module hidden if there's no data at all). `weather.sh show` (the on-click) opens the full forecast.
...
- **Built-in tooltips:** `memory` hover shows used/total/free + swap (`tooltip-format` with `{avail}`/`{swap*}`); `clock` hover shows the full date (`%A, %d %B %Y`, bold) + a **native month calendar** (`{calendar}` in `<tt>`, today accented `#ce0056`) — waybar v0.12, no external `cal`. The `calendar` needs the `actions` (`on-scroll` shift month) to render — that's also what hover-scroll uses to flip months. The hand cursor those bring is now backed by an `on-click` that pops a full-year calendar (`python3 -m calendar | less` in a floatterm, toggled — no package).
