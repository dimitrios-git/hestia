" hestia.vim — hestia's editor colorscheme.
"
" Origin: inspired by the `wildcharm` colorscheme (Maxim Kim, in the official
" vim/colorschemes repo; palette derived from "quiet" by @neutaaaaan) — the
" scheme hestia's whole theme was built on. Through palette 0.7.x hestia.vim was
" a thin wrapper that loaded wildcharm at runtime and overrode two lines; THIS
" file is where hestia owns its editor scheme outright: self-contained, no
" runtime dependency, and identical in Vim and Neovim. Seeded from wildcharm's
" Vim 9.1 copy and diverging from here (Claude-style diff fills, treesitter/LSP
" groups, a palette-driven render — see docs/theme-roadmap.md). Credit to
" wildcharm is inspiration, preserved in the git history, not a live dependency.
"
" Phase 1 (this commit): the wildcharm seed made self-contained, with hestia's
" existing deviations applied and Vim/Neovim unified — NO colour changes yet:
"   - ground lifted to the hestia #1a1a1a dark / #f5f5f5 light (upstream is pure
"     #000000 / #ffffff) and primary text to #e0e0e0 / #1a1a1a — the 0.5.0
"     bg-lift + M7 light desktop (matches kitty/waybar/vifm/roles.text);
"   - Neovim parity: source Vim's default group baseline so anything we do not
"     set explicitly looks identical in both editors, and mirror
"     g:terminal_ansi_colors into Neovim's g:terminal_color_* (:terminal).
" Later phases evolve the colours from palette.yml (the SSOT). Everything below
" the header/init is still the wildcharm seed, edited only where noted.

hi clear
if has('nvim')
  " Match Vim's default group baseline in Neovim so groups we don't set here
  " render identically in both editors (upstream's Nvim wildcharm does the same).
