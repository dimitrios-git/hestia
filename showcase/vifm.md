...
- **Re-opening is idempotent, and other imvs are sacred** — another Enter in
  vifm retargets the running imv instead of stacking a second window; Enter
  in a *different* folder retires the old session and starts one there. Each
  session owns its imv **by PID** (a neat trick: the launcher `exec`s imv,
  so the launcher's own PID *becomes* imv's), never by process name — so a
  standalone imv, or a browse session in another vifm window, is invisible
  to it. All session state is namespaced per vifm instance, which means two
  vifm windows can run two full browse sessions side by side, each with its
  own imv, sync, and watcher.

### How it got this way: a week of live fire

The bridge above reads like a design; it is actually a fossil record. One
week of daily use against real media collections broke it seven different
ways, and each break taught the design something:

1. A **2000-video folder** locked the whole flow for minutes — the launcher
   generated every poster up front. Lesson baked in: *the selected file
   first, the session second, thumbnails last* (cursor-first + idle backfill).
2. Still slow: building imv's argument list spawned six processes per file —
   7 seconds of pure fork overhead with zero thumbnail work. One batch pass.
3. Still seconds: the shell's incremental argument loop was accidentally
   O(N²). A one-shot split made it free.
4. **Posters against the wrong videos** — imv silently *drops* list entries
   it can't decode, shifting every index after a corrupt file. The session
   map is keyed by display path now; indexes are never trusted.
5. **The synced cursor hopped** — `sort -V` is not vifm's comparator. The
   order is now generated by the very same glibc function vifm links
   (`strverscmp`, via ctypes, on NFKD-decomposed names), verified
   byte-identical against a live vifm.
6. **"pictures worked, my pictures didn't"** — vifm's remote API returns
   paths shell-escaped, so any folder with a space compared as "a different
   directory" and the watcher closed imv on arrival. Unescape, then compare.
7. **Opening a preview in a second vifm closed the first imv** — "find our
   imv" was a `pgrep`, which finds *anyone's* imv. Hence PID ownership and
   per-instance namespacing.

None of these is visible in a feature list, and that is the point: the
integration is not the 300 lines it started as, it is the 300 lines plus
seven field repairs — which is what "years of config" actually means.
