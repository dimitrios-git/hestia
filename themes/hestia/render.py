...
def render_vifm(variant: str) -> str:
    ...
    rows = [
        ...
        "\n\" cursor — a quiet grey bar (fires on every move, so it stays muted);",
        "\" selection — the accent FILL (palette.yml's own stated purpose for it),",
        "\" matching TopLineSel/TabLineSel/StatusLine/WildMenu's accent-fill language.",
        "\" Deliberately NOT bright_magenta (the old choice): that's ALSO Socket's and",
        "\" every image extension's colour, so a selected .jpg read identically to any",
        "\" other .jpg, just bold — functionally invisible. Accent fill has no such",
        "\" collision (no filetype pattern uses it) and only fires on deliberate,",
        "\" occasional selection, unlike the cursor, so it doesn't oversaturate the view.",
        hl("CurrLine", "bold", bar_fg, ui_fg),
        hl("Selected", "bold", r["accent_fg"], r["accent"]),
        "\n\" file types",
