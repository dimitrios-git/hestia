...
| Qt / KDE apps (Kdenlive) | hestia KDE colour scheme (`kdeglobals`) | 🟡 | **`qt_theme` role** renders the wildcharm palette as a KDE colour scheme (accent `#d7005f`) into `~/.config/kdeglobals` **and** `~/.local/share/color-schemes/Hestia.colors`. KDE-Frameworks apps read the colours directly (KColorScheme) and list the scheme (KColorSchemeManager — e.g. **Kdenlive**'s theme selector); no Kvantum, no root, no env var. Chose the colour-scheme route over Kvantum — a KDE app needs only the scheme to hit exact `#d7005f`. **Bare-Qt (non-KDE) apps** (qBittorrent, VLC's Qt UI, …) ignore `kdeglobals` and still need a **qt6ct** platform-theme + `QT_QPA_PLATFORMTHEME` in the session (⬜) — deferred until such an app lands. If Kdenlive doesn't pick up the scheme automatically, select **Hestia** once from its theme menu |
