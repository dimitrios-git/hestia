...
| `enable_android_dev` | `android_studio` | `android: [openjdk-17-jdk, cpu-checker]` | Android development — JDK + `cpu-checker` (apt), **Android Studio** itself from a pinned/sha256-verified vendor tarball into `~/.local/share/android-studio` (no apt package exists), and the `kvm` group (needs a re-login) for accelerated emulation (**default off** — per-host dev-toolchain choice). See `roles/android_studio/README.md` |
