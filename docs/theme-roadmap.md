...
| vim / nvim (standalone) | the same generated `colors/hestia.vim`, mirrored verbatim (+ README, install docs, rig-shot screenshots) | **hestia.vim** — [`dimitrios-git/hestia.vim`](https://github.com/dimitrios-git/hestia.vim), the standalone repo for non-hestia users | ✅ live at v0.10.1 (2026-07-19); the repo tag tracks the palette version — **re-mirror + tag on every palette bump** (colour PRs there are redirected here to palette.yml) |
