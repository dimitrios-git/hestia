...
# Created 0600 BEFORE anything is written: this file carries real credentials
# (immich_db_password, duckdns_token), and the default umask would leave it
# world-readable. Gitignore keeps it out of the public repo; the mode keeps it
# away from other local users.
write_answers() {
    : > "$1" && chmod 600 "$1"
    cat > "$1" <<EOF
---
...
enable_immich: $enable_immich
enable_duckdns: $enable_duckdns
immich_domain: "$immich_domain"
# Secrets — this file is gitignored; that is what keeps them out of the public repo.
...
def_plex=$(cur enable_plex); def_plex=${def_plex:-false}
def_stash=$(cur enable_stash); def_stash=${def_stash:-false}
def_immich=$(cur enable_immich); def_immich=${def_immich:-false}
def_duckdns=$(cur enable_duckdns); def_duckdns=${def_duckdns:-false}
def_immichdomain=$(cur immich_domain)
def_duckdomains=$(cur duckdns_domains)
# SECRETS: carried forward from the existing file, never re-prompted blindly.
# Regenerating immich_db_password against an EXISTING Postgres volume would lock
# Immich out of its own database — so an existing value is always preserved.
def_immichpw=$(cur immich_db_password)
def_ducktoken=$(cur duckdns_token)
...
askyn enable_tci         "Install thecodingidiot chapter prerequisites? (SDL2 headers, gdb, valgrind, alsa-utils, nethack + Mega Drive/SNES/N64/Dreamcast emulators (all BIOS-free) + the Mega Drive gendev toolchain)" "$def_tci"
...
if [ "$enable_docker" = true ]; then
    ...
    askyn enable_stash   "  Deploy the Stash container? (/opt/stash)" "$def_stash"
    askyn enable_immich  "  Deploy the Immich photo server? (/opt/immich)" "$def_immich"
    if [ "$enable_immich" = true ]; then
        # Preserve an existing password; generate one only on a fresh setup. A new
        # password against an existing Postgres data dir would lock Immich out.
        if [ -n "$def_immichpw" ]; then
            immich_db_password=$def_immichpw
            echo "    (keeping the existing Immich database password)"
        else
            immich_db_password=$(tr -dc 'A-Za-z0-9' </dev/urandom | head -c 32)
            echo "    (generated a new 32-char Immich database password)"
        fi
        echo "    Public hostname for TLS. BLANK = LAN-only, no Caddy, nothing exposed."
        echo "    Setting one means forwarding ports 80 AND 443 at your router."
        ask immich_domain "    Immich public domain (blank = LAN-only)" "$def_immichdomain"
    else
        immich_db_password=$def_immichpw; immich_domain=$def_immichdomain
    fi
else
    enable_plex=false; enable_stash=false; enable_immich=false
    immich_db_password=$def_immichpw; immich_domain=$def_immichdomain
fi
askyn enable_duckdns     "Set up DuckDNS dynamic DNS? (also supplies the hostname TLS needs)" "$def_duckdns"
if [ "$enable_duckdns" = true ]; then
    ask duckdns_domains  "  DuckDNS subdomain (without .duckdns.org)" "$def_duckdomains"
    ask duckdns_token    "  DuckDNS token (from duckdns.org)"         "$def_ducktoken"
else
    duckdns_domains=$def_duckdomains; duckdns_token=$def_ducktoken
fi
