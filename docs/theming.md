...
| VS Code | theme extension (`user/vscode/hestia/`) | 🟡 | dark + light themes **GENERATED** by `render.py`; dark = full role-mapped UI chrome + ANSI-16 terminal, light = editor-only until the light desktop ramp (roadmap M7). Not bootstrap-wired (VS Code isn't base-system) — install per the extension README; not yet verified in a live VS Code |
