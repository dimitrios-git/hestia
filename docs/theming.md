...
| ls / file listing | `dircolors` (LS_COLORS) | ✅ | ✅ n/a | `user/bash/.dircolors` — file-**type** colours mirror vifm's highlights exactly (same 256-indices: dir 39, link 44, exec 41, …) so `ls` and vifm agree; plus a restrained wildcharm extension layer (archives/images/video/audio/docs). `.bashrc` already loads it. **Variant-invariant by design** (256-cube indices render the same on either terminal bg) — note vifm's LIGHT colorscheme deviates from these for readability, so `ls` and vifm agree exactly on dark only (roadmap backlog) |
